...
export function GlobalSearchResults({ onClose, attached }: { onClose: () => void; attached?: boolean }) {
  ...
  // Files AND commands share this one dropdown (the search box IS the command
  // palette). With no query we show ALL commands ("Jump to …", Slack-style); with
  // a query we filter commands and add matching files from every drive.
  const hasQuery = q.trim().length > 0;
  const allCommands = useCommands();
  const commands = hasQuery ? filterCommands(allCommands, q) : allCommands;
  ...
  const runCommand = (c: { run: () => void }) => {
    c.run();
    useSearch.getState().set(""); // a nav command dismisses the search
    onClose();
  };
    ...
    <div className={`flex max-h-[min(70vh,560px)] min-h-0 flex-col overflow-hidden border border-[var(--line)] bg-[var(--card)] shadow-[var(--shadow-lg)] ${attached ? "rounded-b-[14px] rounded-t-none border-t-0" : "rounded-[14px]"}`}>
      ...
      {hasQuery && (
        <div className="flex flex-wrap items-center gap-2 border-b border-[var(--line)] px-4 py-2.5 text-[13px]">
          <FileSearch size={16} className="text-[var(--acc)]" />
          <span className="truncate text-[var(--text-2)]">Search {scopeLabel}</span>
          {loading && <Loader2 size={14} className="animate-spin text-[var(--faint)]" />}
          <div className="ml-auto flex items-center gap-1.5">
            {available.map((s) => {
              ...
              return (
                ...
                >
                  <meta_.Icon size={11} /> {meta_.label}
                </button>
              );
            })}
          </div>
      ...
      <div className="min-h-0 flex-1 overflow-auto p-2">
        {/* Commands / quick nav — all of them on focus, filtered as you type. */}
        {commands.length > 0 && (
          <div className="mb-1">
            <div className="px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.05em] text-[var(--faint)]">{hasQuery ? "Commands" : "Jump to"}</div>
            {commands.map((c) => (
              <button
                key={c.id}
                onClick={() => runCommand(c)}
                className="flex w-full items-center gap-3 rounded-[9px] px-3 py-2 text-left text-[13.5px] text-[var(--text-2)] hover:bg-[var(--hover)] hover:text-[var(--text)]"
              >
        ...
        {/* Everything below is file search — only relevant once there's a query. */}
        {hasQuery && (
          <>
            {scope === "folder" && !folderRecursive && scopeAccount && folderPath && (
              <button
                onClick={() => void useIndex.getState().indexFolder(scopeAccount, folderPath)}
                className="mb-2 flex w-full items-center gap-2 rounded-[9px] border border-dashed border-[var(--line2)] px-3 py-2 text-left text-[12.5px] text-[var(--faint)] hover:text-[var(--text-2)]"
              >
            ...
            {loading && results.length === 0 ? (
              ...
              <div className="flex flex-col items-center justify-center gap-2 py-12 text-center text-sm text-[var(--mut)]">
              ...
              groups.map(([id, groupHits]) => (
                <div key={id || "unknown"} className="mb-2">
                  <div className="sticky top-0 z-10 flex items-center gap-2 bg-[var(--card)] px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.05em] text-[var(--faint)]">
